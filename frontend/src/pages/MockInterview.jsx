...
const MockInterview = () => {
    ...
    const submitAnswer = async () => {
        ...
        // 🚀 OPTIMISTIC NAVIGATION: Move to next immediately for smooth experience
        ...
        try {
            const { data } = await api.post(
                ...
                { questionIndex: currentQIndex, answer: currentAnswer }
            );

            // Store local answer for redundancy
            setAnswers(prev => [...prev, {
                question: interview.questions[currentQIndex].question,
                answer: currentAnswer
            }]);

            // Final check if this was the last question
                ...
                <div className="glass-card" style={{ padding: '3rem' }}>
                    ...
                    <div style={{ textAlign: 'center', margin: '3rem 0' }}>
                        <div style={{ fontSize: '4rem', fontWeight: '800', color: '#6366f1' }}>
                            {finalResults.scores.overall}/10
                        </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
                        <div className="score-stat-card">
                            <div className="score-stat-value">{finalResults.scores.confidence}</div>
                            <div className="score-stat-label">Confidence</div>
                        </div>
                        <div className="score-stat-card">
                            <div className="score-stat-value">{finalResults.scores.clarity}</div>
                            <div className="score-stat-label">Clarity</div>
                        </div>
                        <div className="score-stat-card">
                            <div className="score-stat-value">{finalResults.scores.accuracy}</div>
                            <div className="score-stat-label">Accuracy</div>
                        </div>
                        <div className="score-stat-card">
                            <div className="score-stat-value">{finalResults.scores.communication}</div>
                            <div className="score-stat-label">Communication</div>
                        </div>
                    </div>

                    {/* Detailed Question-wise Evaluation */}
                    <h2 style={{ marginBottom: '1.5rem' }}>Detailed Report</h2>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: '3rem' }}>
                        {finalResults.questions.map((item, index) => (
                            <div key={index} className="glass-card evaluation-item" style={{ padding: '1.5rem', background: 'var(--bg-secondary)' }}>
                                ...
                                <div className="evaluation-section">
                                    <div className="evaluation-label">Your Answer:</div>
                                    <div className="evaluation-content">{item.userAnswer}</div>
                                </div>

                                <div className="evaluation-section feedback">
                                    <div className="evaluation-label">AI Feedback & Strengths:</div>
                                    <div className="evaluation-content">{item.aiEvaluation || 'Evaluation pending...'}</div>
                                </div>
                            </div>
                        ))}
                    </div>
